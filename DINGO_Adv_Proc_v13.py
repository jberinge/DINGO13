...
#
# Programmed by Jason Beringer updated 5/10/2016
############################################################################
...
import BIOS_nc_fetch_v6 as CABLE_nc_fetch
...
import Gap_Fill_climatology_v2 as climatology

#Add a path so that we can read the python scripts from in Ians Analysis tools directory
sys.path.insert(0, '../Analysis_tools/')

import uncertainty_run as Uncertainty
...
def main():
	...
	# Option to save original flux dataframe. Uncomment
	...
	AWS_combined.to_pickle(myBaseforResults+'/AWS/AWS_combined_'+Site_ID+'.df') 
	AWS_combined.to_csv(myBaseforResults+'/'+'/AWS/AWS_combined_'+Site_ID+'.csv', sep=',')
	...
	#Here create a gap filled EVI for later use.  Later on use '250m_16_days_EVI_new_interp_Con'.  Need to create a QC flag as well 
	New_combined['250m_16_days_EVI_new_interp_Con'] = New_combined['250m_16_days_EVI_new_interp']
	New_combined['250m_16_days_EVI_new_interp_Con_QCFlag']=np.nan
	New_combined['250m_16_days_EVI_new_interp_Con_QCFlag'][New_combined['250m_16_days_EVI_new_interp_Con'].notnull()]=1
	New_combined=climatology.climatology_monthly_diurnal(New_combined,'250m_16_days_EVI_new_interp')
	#Save new data
	New_combined.to_pickle(DFfilename) 
	New_combined.to_csv(CSVfilename)	
	...
	list_in=['Fsd_Con','VPD_Con','Sws_Con','Ts_Con',Ws_variable_name+'_Con','250m_16_days_EVI_new_interp_Con']
	target=['Fc']
	#Call ANN routines
	...
	print "====================================================================="
	print "DINGO: Starting Barr et al.  Get some coffee!"
	print "====================================================================="
	Fluxfilepath=mypathforFluxdata+'/'+cf['Files']['FLUXfilename']
	...
	#Sometimes there are years missing from the annual stats sheet.  Average ustar and gap fill any missing values 
	...
	#Call as many times as needed for each variable or set of variables
	list_in=['Sws_Con','Ts_Con','Ta_Con','250m_16_days_EVI_new_interp_Con']
	list_out=['Fc']
    ...
    if cf['Options']['calculate_uncertainty']=='Yes':     	
	#set variables
	NEE_trials=int(cf['Options']['number_of_iterations'])
	prompt_for_config = False
	output_trial_results = True
	output_plot = True
	#Get CPD data from saved results
	CPDdata = pd.read_csv(myBaseforResults + '/CPD/Results/annual_statistics.csv')	
	# Load in previous data step data
	New_combined=pd.read_pickle(DFfilename) 	
	
	#For Ians code needs Fc_storage_obs variable
	New_combined['Fc_storage_obs']=np.nan
	
	New_combined['VPD']=New_combined['VPD_Con']
	
	New_combined.to_pickle(DFfilename) 	
	
	
	#Check for directory
	uncertainty_dir=myBaseforResults+'/Diagnostics/Uncertainty/'
	if not os.path.isdir(uncertainty_dir):
	    os.mkdir(uncertainty_dir)
	CPD_filename = myBaseforResults+'/CPD/Results/annual_statistics.csv'
	
	#Call main code and return two dictionaries
	print "Calculating uncertainty results will take a very longtime!!!!"
	final_rslt_dict, plot_dict = Uncertainty.main(DFfilename, CPD_filename, 30, uncertainty_dir, output_plot = True)
	
	#Take outputs and write them 
	
	#Do this by year read years from key
	for this_year in final_rslt_dict.keys(): 
	    with open(myBaseforResults+'/Diagnostics/Uncertainty/Uncertainty_results_' + str(this_year) +'.csv', 'wb') as f:  # Just use 'w' mode in 3.x
	    ...
	    with open(myBaseforResults+'/Diagnostics/Uncertainty/Uncertainty_summary_' + str(this_year) +'.csv', 'wb') as f:  # Just use 'w' mode in 3.x
