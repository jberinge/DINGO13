...
def fetch_CABLE(FLUXDataframe,myBaseforResults,CABLEfilename,Site_ID,latitude,longitude):
    ...
    print "============================================="
    print "DINGO: STARTING BIOS/CABLE IMPORT"
    print "============================================="    
    ...
    #Was 2000 for 2014 BIOS runs but for 2016 it now goes back to 1900
    ...
    rng = netCDF4.num2date(times[:],times.units)
    ...
    #Confine to just times present in Fluxdata
    Flux_start=FLUXDataframe.index[0]
    Flux_finish=FLUXDataframe.index[-1]
    theDataFrame=theDataFrame[Flux_start:Flux_finish]

    #round DF to 1 hour   
    theDataFrame.index = theDataFrame.index.round('1h')
    
    #then sort and check for duplicates
    theDataFrame.sort_index(inplace=True)

    #theDataFrame["index"] = theDataFrame.index
    #theDataFrame.drop_duplicates('index', inplace=True)
    #del theDataFrame["index"]

    #Convert to 30 minute freq and interpolate
    theDataFrame=theDataFrame.asfreq('30Min')
    theDataFrame = theDataFrame.interpolate(method='linear')
    
    #Write the solar noon to the DF so we can use it later
    theDataFrame['Solar_noon']=np.nan

    def dotheshift(group):
	...
	#add an extra interval to account for Bom versus other 30 minute offset
	intervals=int(round(time_difference_minutes/30))
	#pdb.set_trace()
	#Now fore each 30  minutes we shift
	group = group.shift(intervals)
	...
	dailyCABLE_DF_grouped=theDataFrame.groupby([lambda x: x.year, lambda x: x.month,lambda x: x.day]).apply(dotheshift)

	
	print "Average of Solar noon " + str(dailyCABLE_DF_grouped['Solar_noon'].mean())
    ...
    # Save dataframe. 
    #CABLEandTOWER_DF.to_pickle(mypathforResults+'CABLEandTOWER_DF_for_'+Site_ID+'.df')  
    ...
    print "============================================="
    print "DINGO: FINISHED CABLE IMPORT"
    print "============================================="
