...
def get_data(configs_dict, DataDF = None):
    #Check to see of dataframe is passed
    ...
    if ext == '.nc':
        ...
        data_dict, attr = io.DINGO_df_to_data_structure(DataDF, data_input_target,
...
#------------------------------------------------------------------------------
def plot_data(data_d,configs_dict,myBaseforResults = None, Site_ID = None, versionID = None):
    """
    ...
    for this_year in data_d.keys():
        ...
        ax1.text(xmin + (xmax - xmin) / 10, ymax - (ymax - ymin) / 20, 
                 str(this_year) + ' ' + Site_ID + ' '+ versionID, fontsize = 20)
        ...
        try:
            fig.savefig(myBaseforResults+'/Diagnostics/Uncertainty/Uncertainty plot for '+str(this_year) + '_' + Site_ID + '_'+ versionID)
        except:
            print "Could not plot year " + str(this_year)
        print "finish plot for " + str(this_year)
        
    return    

def main(NEE_trials=None, CPDdata= None, DataDF= None, prompt_for_config = False, output_trial_results = True, output_plot = True, myBaseforResults = None, Site_ID = None, versionID = None):
    ...
    if prompt_for_config == True:
        configs_master_dict = io.config_to_dict(io.file_select_dialog())
    else:
        configs_master_dict = io.config_to_dict('./Analysis_tools/Uncertainty_configs.txt')
    
    # Build custom configuration file for this script
    configs_dict = build_config_file(configs_master_dict)

    # Get the data
    #Send dataframe if passed
    data_dict, attr = get_data(configs_dict, DataDF)
    ...
    for this_year in years:
        ...
        if not CPDdata.empty:
            #Test to see if the year is in the file
            try:
                ustar_threshold = float(CPDdata['ustar_mean'][CPDdata['Unnamed: 0'] == this_year].values)
                ustar_uncertainty = float(CPDdata['ustar_sig'][CPDdata['Unnamed: 0'] == this_year].values)                       
            except:            
                ustar_threshold = float(CPDdata['ustar_mean'].mean())
                ustar_uncertainty = float(CPDdata['ustar_sig'].mean())    
        else:
...
#        print 'Maximum ustar for year ' + str(this_year) + ' is ' + str(round(max_ustar, 3))                          
        ...
        # Ouput plots
        if output_plot:
            plot_data({this_year: interm_rslt_dict},configs_dict, myBaseforResults ,Site_ID, versionID)
